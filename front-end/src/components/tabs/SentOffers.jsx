...
export default function SentOffers() {
  ...
  const emptyMessage = () => {
    if (outcomeFilter === 'pending') return 'لا توجد عروض بانتظار رد الطالب.';
    if (outcomeFilter === 'contact_shared') return 'لا توجد عروض تم التواصل فيها بعد.';
    if (outcomeFilter === 'rejected') return 'لا توجد عروض مرفوضة.';
    if (outcomeFilter === 'closed') return 'لا توجد عروض منتهية.';
    return 'لم ترسل أي عروض أو ردود بعد.';
  };

  return (
    <div className="page-container2 ohub-page-root">
      <div className="requests-tab-container ohub-page">
        <div className="ohub-hero ohub-hero-sent">
          <button
            type="button"
            className="ohub-back-btn"
            onClick={() => navigate('/dashboard/offers')}
          >
            <FaArrowRight /> رجوع لتواصلي
          </button>
          <div className="tab-page-header">
            <h2>عروضي المرسلة للطلاب</h2>
            <p>كل ما أرسلته — معلّق، تم التواصل، مرفوض، أو منتهٍ</p>
          </div>
          {!isLoading && allItems.length > 0 && (
            <div className="ohub-sent-legend">
              <span className="ohub-legend-item exchanged">تم أخذ الرقم</span>
              <span className="ohub-legend-item waiting">بانتظار / بدون رقم</span>
            </div>
          )}
        </div>
        ...
        {!isLoading && allItems.length > 0 && (
          <div className="ohub-outcome-pills">
            {HUB_FILTERS.map((f) => (
              ...
              >
                {f.label}
                {f.value === 'contact_shared' && stats.shared > 0 && ` (${stats.shared})`}
        ...
        {isLoading && <p className="loading-text">جارِ تحميل العروض...</p>}
        {error && <p className="error-text">{error}</p>}

        {!isLoading && !error && allItems.length === 0 && (
          <div className="contacts-empty ohub-empty">
            <strong>لم ترسل عروضاً بعد</strong>
            <p>تصفّح الطلبات العامة أو رد على طلبات خاصة لبدء إرسال العروض.</p>
            <button type="button" className="ohub-sent-link-btn" onClick={() => navigate('/dashboard/requests')}>
              تصفّح الطلبات العامة
            </button>
          </div>
        )}
