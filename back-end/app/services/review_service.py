...
def get_reviews_by_student(db: Session, student_id: int) -> List[Review]:
    return db.scalars(select(Review).where(Review.student_id == student_id)).all()


def get_reviews_by_tutor(db: Session, tutor_id: int) -> List[Review]:
    return db.scalars(select(Review).where(Review.tutor_id == tutor_id)).all()
...
def get_all_reviews_out(db: Session) -> List[ReviewOut]:
    ...
    return [_review_to_out(review) for review in reviews]
