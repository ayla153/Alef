...
class PostStatus(Base):
    ...
    #foreign keys
    post_requirements_id: Mapped[int] = mapped_column(ForeignKey("post_requirements.post_requirements_id"), nullable=False)
    tutor_id: Mapped[int] = mapped_column(ForeignKey("tutors.tutor_id"), nullable=False)

    #relationships
    posts : Mapped["PostRequirement"] = relationship("PostRequirement", back_populates="post_status")
